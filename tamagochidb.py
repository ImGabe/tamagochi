...
cursor.execute(
    'CREATE TABLE IF NOT EXISTS  tamagochi (ID INT PRIMARY KEY UNIQUE, Nome VARCHAR (255) UNIQUE NOT NULL, lvl INT NOT NULL)')
...
def adicionar(nome, lvl):
    cursor.execute(
        "INSERT INTO tamagochi (Nome, Lvl) VALUES (?,?)", (nome, lvl))
    conn.commit()
    conn.close()
